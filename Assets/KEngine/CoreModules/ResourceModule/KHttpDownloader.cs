...
public class KHttpDownloader : MonoBehaviour, IDisposable
{
    ...
    public static KHttpDownloader Load(string fullUrl, string toPath, bool useCache = false, int expireDays = 1, int timeout = 5)
    {
        var downloader = new GameObject("HttpDownlaoder+"+fullUrl).AddComponent<KHttpDownloader>();
    ...
    private void Init(string fullUrl, string toPath, bool useCache = false, int expireDays = 1, int timeout = 5)
    {
        ...
        KResourceModule.Instance.StartCoroutine(StartDownload(fullUrl));

    }

    public static KHttpDownloader Load(string fullUrl, string toPath, int expireDays, int timeout = 5)
    {
    ...
    IEnumerator StartDownload(string fullUrl)
    {
        ...
        if (UseCache && File.Exists(_SavePath))
        {
            ...
            Logger.Log("缓存文件: {0}, 最后修改时间: {1}", _SavePath, lastWriteTime);
            var deltaDays = CTool.GetDeltaDay(lastWriteTime);
            // 文件未过期
            if (deltaDays < ExpireDays)
            {
                Logger.Log("缓存文件未过期 {0}", _SavePath);
                FinishedFlag = true;
                ErrorFlag = false;
                yield break;
            }
...
//#endif
                Logger.LogError("[KHttpDownloader]下载线程超时！: {0}", fullUrl);
                isThreadError = true;
                break;
            }
        ...
        if (isThreadError)
        {
            Logger.LogError("Download WWW Error: {0}", fullUrl);
            ErrorFlag = true;
    ...
    {
        Logger.Assert(IsFinished);
        Logger.Assert(!IsError);
    ...
    void ThreadableResumeDownload(string url, Action<int, int> stepCallback, Action errorCallback, Action successCallback)
    {
        ...
        if (System.IO.File.Exists(TmpDownloadPath))
        {
            ...
            Logger.LogConsole_MultiThread("Resume.... from {0}", lStartPos);
        }
        else
        {
        ...
        try
        {
            ...
            Logger.LogConsole_MultiThread("Getting Response : {0}", url);

            //向服务器请求，获得服务器回应数据流 
            using (var response = request.GetResponse())  // TODO: Async Timeout
            {
                Logger.LogConsole_MultiThread("Getted Response : {0}", url);
                if (IsFinished)
                {
                ...
                else
                {
                    ...
                    using (var ns = response.GetResponseStream())
                    {
                        ...
                        Logger.LogConsole_MultiThread("Start Stream: {0}", url);
            ...
            Logger.LogConsole_MultiThread("下载完成: {0}", url);
        ...
        {
            Logger.LogConsole_MultiThread("下载过程中出现错误:" + ex.ToString());
            downloadFileStream.Close();
            ...
            {
                Logger.LogConsole_MultiThread(e.Message);
            }
    ...
    void OnDestroy()
    {
        if (!FinishedFlag)
        {
            FinishedFlag = true;
            ErrorFlag = true;
            Logger.LogError("[HttpDownloader]Not finish but destroy: {0}", Url);
        }
    }
